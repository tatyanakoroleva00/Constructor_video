...
const CoursesButton = ({ btnIndex, setCurrentInteractive, index, setActiveBtn, activeBtn, interactivesNamesArr, serverData, serverDataGot }) => {
  return (
    ...
    <>
      ...
      {serverDataGot && <button className={`${activeBtn === index ? styles['active-btn'] : styles['courses-btn']}`} value={btnIndex} onClick={(event) => { setCurrentInteractive(btnIndex); setActiveBtn(index); }}>{serverData['interactives'][index] ? serverData['interactives'][index]['data']['interactive_name'] : interactivesNamesArr[index] }</button>}
    </>
  )
}
